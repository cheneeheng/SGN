...
class NTUDataLoaders(object):
    ...
    def get_val_loader(self, batch_size, num_workers):
        if self.dataset == 'NTU' or self.dataset == 'kinetics' or self.dataset == 'NTU120':  # noqa
    ...
    def create_datasets(self):
        ...
        with open(path + '_train_label.pkl', 'rb') as fr:
            self.train_Y = pickle.load(fr)
        with open(path + '_val.pkl', 'rb') as fr:
            self.val_X = pickle.load(fr)
        with open(path + '_val_label.pkl', 'rb') as fr:
            self.val_Y = pickle.load(fr)
        with open(path + '_test.pkl', 'rb') as fr:
            self.test_X = pickle.load(fr)
        with open(path + '_test_label.pkl', 'rb') as fr:
            self.test_Y = pickle.load(fr)
        # f = h5py.File(path, 'r')
        # self.train_X = f['x'][:]

...
# - 2X TODO that are unsolved.
# - read raw data
# - get either sequence for 1 actor or 2 actors
# - if 2 actors, denoise data first
# - data denoised by length,x-y spread, and 2 bodies with highest motion (var)
# - discard frames with all zeros, either only looking at actor 1 or
# looking at both actors


def denoising_by_length(ske_name: str, bodies_data: dict) -> Tuple[dict, str]:
    """
    Denoising data based on the frame length for each bodyID.
    Filter out the bodyID which length is less or equal than the
...
def get_valid_frames_by_spread(points: np.ndarray) -> list:
    """
    Find the valid (or reasonable) frames (index)
    based on the spread of X and Y.
...
def denoising_by_spread(ske_name: str,
                        bodies_data: dict) -> Tuple[dict, str, bool]:
    """
    Denoising data based on the spread of Y value and X value.
    Filter out the bodyID which the ratio of noisy frames is higher than
    the predefined threshold.

    bodies_data: contains at least 2 bodyIDs
    """
    # mark if this sequence has been processed by spread.
    ...
    for (bodyID, body_data) in new_bodies_data.items():
        ...
        raw_joints = body_data['joints'].reshape(-1, 25, 3)
        valid_frames = get_valid_frames_by_spread(raw_joints)
        ...
        if ratio >= noise_spr_thres2:  # 0.69754
            ...
            noise_info += 'Filter out: %s (spread rate >= %.2f).\n' % (bodyID, noise_spr_thres2)  # noqa
            noise_spr_logger.info('%s\t%s\t%.6f\t%.6f' % (ske_name, bodyID, motion, ratio))  # noqa
        else:  # Update motion
            joints = raw_joints[valid_frames]
            body_data['motion'] = min(motion, np.sum(np.var(joints.reshape(-1, 3), axis=0)))  # noqa
            noise_info += '%s: motion %.6f -> %.6f\n' % (bodyID, motion, body_data['motion'])  # noqa
...
def denoising_by_motion(ske_name: str,
                        bodies_data: dict,
                        bodies_motion: dict) -> Tuple[list, str]:
    """
    Filter out the bodyID which motion is out of the range of a
    predefined interval.

    """
    # Sort bodies based on the motion, return a list of tuples
    # bodies_motion = sorted(bodies_motion.items(),
    #                        key=lambda x, y: cmp(x[1], y[1]), reverse=True)
    bodies_motion = sorted(bodies_motion.items(), key=lambda x: x[1],
                           reverse=True)

    # Reserve the body data with the largest motion
    denoised_bodies_data = [(bodies_motion[0][0], bodies_data[bodies_motion[0][0]])]  # noqa
    noise_info = str()
    ...
    for (bodyID, motion) in bodies_motion[1:]:
        if (motion < noise_mot_thres_lo) or (motion > noise_mot_thres_hi):
            ...
            noise_mot_logger.info('{}\t{}\t{:.6f}'.format(ske_name, bodyID, motion))  # noqa
        else:
...
def denoising_bodies_data(bodies_data: dict) -> Tuple[list, str]:
    """
    Denoising data based on some heuristic methods, not necessarily correct
    for all samples.
    ...
    bodies_data, noise_info_spr, denoised_by_spr = \
        denoising_by_spread(ske_name, bodies_data)
    ...
    # Step 3: Sort bodies based on the motion
    # bodies_motion = sorted(bodies_motion.items(),
    #                        key=lambda x, y: cmp(x[1], y[1]), reverse=True)
    bodies_motion = sorted(bodies_motion.items(), key=lambda x: x[1],
                           reverse=True)
    ...
    # bodies_data, noise_info = \
    #     denoising_by_motion(ske_name, bodies_data, bodies_motion)

    # return bodies_data, noise_info


def remove_missing_frames(ske_name: str,
                          joints: np.ndarray,
                          colors: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    Cut off missing frames which all joints positions are 0s

    For the sequence with 2 actors' data, also record the number of
    missing frames for actor1 and actor2, respectively (for debug).
    """
    ...
    # For two-subjects action, this means both data of actor1 and actor2
    # is missing.
...
def get_one_actor_points(body_data: dict,
...
def get_bodies_info(bodies_data: dict) -> str:
...
def get_two_actors_points(bodies_data: dict) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    # Arguments:
        ...
        bodies_data['data'] is also a dict, while the key is bodyID, the value
        is the corresponding body_data which is also a dict with 4 keys:
    ...
    bodies_data, noise_info = denoising_bodies_data(bodies_data)  # Denoise
    ...
    if len(bodies_data) == 1:  # Only left one actor after denoising
        ...
        info2 = '{}\t{:^8}\t{:f}\n'.format(bodyID, str([start1, end1]), actor1['motion'])  # noqa
        actor1_info = info1 + info2
        del bodies_data[0]

        actor2_info = '{:^17}\t{}\t{:^8}\n'.format('Actor2', 'Interval', 'Motion')  # noqa
...
def get_raw_denoised_data():
    """
    Get denoised data (joints positions and color locations) from raw skeleton
    sequences.

    For each frame of a skeleton sequence, an actor's 3D positions of 25 joints
    represented by an 2D array (shape: 25 x 3) is reshaped into a 75-dim vector
    by concatenating each 3-dim (x, y, z) coordinates along the row dimension
    in joint order. Each frame contains two actor's joints positions
    constituting a 150-dim vector. If there is only one actor, then the
    last 75 values are filled with zeros. Otherwise, select the main actor and
    the second actor based on the motion amount. Each 150-dim vector as
    a row vector is put into a 2D numpy array where the number of rows equals
    the number of valid frames. All such 2D arrays are put into a list and
    finally the list is serialized into a cPickle file.

    For the skeleton sequence which contains two or more actors
    (mostly corresponds to the last 11 classes), the filename and
    actors' information are recorded into log files.
    For better understanding, also generate RGB+skeleton videos
    for visualization.
    """
    # load raw skeletons data
    ...
    for (idx, bodies_data) in enumerate(raw_skes_data):
        ...
        if num_bodies == 1:  # only 1 actor
            ...
            # more than 1 actor, select two main actors
...
if __name__ == '__main__':
    ...
    missing_count = 0  # number of missing skeletons, i.e. zeros frame
    noise_len_thres = 11  # threshold to delete skeleton
    noise_spr_thres1 = 0.8  # threshold for x_max-min/y_max-min ratio
    noise_spr_thres2 = 0.69754  # threshold for noise/total frames ratio
    ...
    noise_len_logger.addHandler(logging.FileHandler(osp.join(save_path, 'noise_length.log')))  # noqa
    noise_len_logger.info('{:^20}\t{:^17}\t{:^8}\t{}'.format('Skeleton', 'bodyID', 'Motion', 'Length'))  # noqa

    noise_spr_logger = logging.getLogger('noise_spread')
    noise_spr_logger.setLevel(logging.INFO)
    noise_spr_logger.addHandler(logging.FileHandler(osp.join(save_path, 'noise_spread.log')))  # noqa
    noise_spr_logger.info('{:^20}\t{:^17}\t{:^8}\t{:^8}'.format('Skeleton', 'bodyID', 'Motion', 'Rate'))  # noqa

    noise_mot_logger = logging.getLogger('noise_motion')
    noise_mot_logger.setLevel(logging.INFO)
    noise_mot_logger.addHandler(logging.FileHandler(osp.join(save_path, 'noise_motion.log')))  # noqa
    noise_mot_logger.info('{:^20}\t{:^17}\t{:^8}'.format('Skeleton', 'bodyID', 'Motion'))  # noqa

    fail_logger_1 = logging.getLogger('noise_outliers_1')
    fail_logger_1.setLevel(logging.INFO)
    fail_logger_1.addHandler(logging.FileHandler(osp.join(save_path, 'denoised_failed_1.log')))  # noqa

    fail_logger_2 = logging.getLogger('noise_outliers_2')
    fail_logger_2.setLevel(logging.INFO)
    fail_logger_2.addHandler(logging.FileHandler(osp.join(save_path, 'denoised_failed_2.log')))  # noqa

    missing_skes_logger = logging.getLogger('missing_frames')
    missing_skes_logger.setLevel(logging.INFO)
    missing_skes_logger.addHandler(logging.FileHandler(osp.join(save_path, 'missing_skes.log')))  # noqa
    missing_skes_logger.info('{:^20}\t{}\t{}'.format('Skeleton', 'num_frames', 'num_missing'))  # noqa

    missing_skes_logger1 = logging.getLogger('missing_frames_1')
    missing_skes_logger1.setLevel(logging.INFO)
    missing_skes_logger1.addHandler(logging.FileHandler(osp.join(save_path, 'missing_skes_1.log')))  # noqa
    missing_skes_logger1.info('{:^20}\t{}\t{}\t{}\t{}\t{}'.format('Skeleton', 'num_frames', 'Actor1', 'Actor2', 'Start', 'End'))  # noqa

    missing_skes_logger2 = logging.getLogger('missing_frames_2')
    missing_skes_logger2.setLevel(logging.INFO)
    missing_skes_logger2.addHandler(logging.FileHandler(osp.join(save_path, 'missing_skes_2.log')))  # noqa
    missing_skes_logger2.info('{:^20}\t{}\t{}\t{}'.format('Skeleton', 'num_frames', 'Actor1', 'Actor2'))  # noqa
